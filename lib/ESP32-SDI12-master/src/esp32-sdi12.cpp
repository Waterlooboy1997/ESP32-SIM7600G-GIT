...
ESP32_SDI12::Status ESP32_SDI12::querySensor(uint8_t address,
    ...
    switch(cmd){
        ...
        case SDI12_CRC_Measure:
            response = true;
            snprintf(sensor_cmd, sizeof(sensor_cmd), "%dM1!", address);
            break;
...
ESP32_SDI12::Status ESP32_SDI12::requestMeasure(uint8_t address,
    ...
    if(command == 1){
        Status res = querySensor(address, SDI12_AD_Measure, 0, 0); //This comment is used for starflow (aMa!)
    } else if (command == 2){
        Status res = querySensor(address, SDI12_Measure, 0, 0); //This comment is used for turbidity sensor (aM!)
    } else if (command == 3){
        Status res = querySensor(address, SDI12_CRC_Measure, 0, 0); //This comment is used for wiping the turbidity sensor (aM1!)
    }
...
ESP32_SDI12::Status ESP32_SDI12::measure(uint8_t address,
    ...
    Status res = requestMeasure(address, command, &measure);
    if (res != SDI12_OK) {
        return res;
    }
